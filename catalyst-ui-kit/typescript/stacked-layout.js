      ...
      <Headless.DialogPanel transition className="fixed inset-y-0 w-full max-w-80 p-2 transition duration-300 ease-in-out data-closed:-translate-x-full">
        <div className="flex h-full flex-col rounded-lg bg-white shadow-xs ring-1 ring-zinc-950/5 dark:bg-zinc-900 dark:ring-white/10">
          <div className="-mb-3 px-4 pt-3">
            <Headless.CloseButton as={navbar_1.NavbarItem} aria-label="Close navigation">
              <CloseMenuIcon />
...
function StackedLayout({ navbar, sidebar, children, }) {
    let [showSidebar, setShowSidebar] = (0, react_1.useState)(false);
    return (<div className="relative isolate flex min-h-svh w-full flex-col bg-white lg:bg-zinc-100 dark:bg-zinc-900 dark:lg:bg-zinc-950">
      ...
      <header className="flex items-center px-4">
        <div className="py-2.5 lg:hidden">
          <navbar_1.NavbarItem onClick={() => setShowSidebar(true)} aria-label="Open navigation">
            <OpenMenuIcon />
          </navbar_1.NavbarItem>
        </div>
