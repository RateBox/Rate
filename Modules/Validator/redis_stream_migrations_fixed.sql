-- Redis Stream Integration Migrations (Fixed)
-- Add support for stream processing metadata

-- Add source column first (missing from original schema)
...
-- Create view for recent stream processing activity (fixed column references)
...
INSERT INTO stream_processing_metrics (stream_name, metric_name, metric_value, tags)
VALUES 
    ('raw_items', 'migration_completed', 1, '{"version": "1.0", "type": "redis_stream_integration"}'::jsonb),
    ('raw_items', 'schema_version', 1, ('{"migration_date": "' || NOW()::text || '"}')::jsonb);

-- Show migration summary
DO $$
BEGIN
    RAISE NOTICE 'Redis Stream Integration Migration Completed';
    RAISE NOTICE 'Added columns: source, stream_id, consumer_group, consumer_name, processing timestamps';
