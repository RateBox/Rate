...
exports.SecondaryFeatures = void 0;
const react_1 = require("react");
const image_1 = __importDefault(require("next/image"));
const react_2 = require("@headlessui/react");
const clsx_1 = __importDefault(require("clsx"));
const Container_1 = require("@/components/Container");
const contacts_png_1 = __importDefault(require("@/images/screenshots/contacts.png"));
const inventory_png_1 = __importDefault(require("@/images/screenshots/inventory.png"));
const profit_loss_png_1 = __importDefault(require("@/images/screenshots/profit-loss.png"));
const features = [
    {
        name: 'Reporting',
        ...
        image: profit_loss_png_1.default,
...
function Feature({ feature, isActive, className, ...props }) {
    return (<div className={(0, clsx_1.default)(className, !isActive && 'opacity-75 hover:opacity-100')} {...props}>
      <div className={(0, clsx_1.default)('w-9 rounded-lg', isActive ? 'bg-blue-600' : 'bg-slate-500')}>
        <svg aria-hidden="true" className="h-9 w-9" fill="none">
          <feature.icon />
        </svg>
      </div>
      <h3 className={(0, clsx_1.default)('mt-6 text-sm font-medium', isActive ? 'text-blue-600' : 'text-slate-600')}>
        {feature.name}
      </h3>
...
function FeaturesMobile() {
    return (<div className="-mx-4 mt-20 flex flex-col gap-y-10 overflow-hidden px-4 sm:-mx-6 sm:px-6 lg:hidden">
      {features.map((feature) => (<div key={feature.summary}>
          ...
          <div className="relative mt-10 pb-10">
            ...
            <div className="relative mx-auto w-211 overflow-hidden rounded-xl bg-white shadow-lg ring-1 shadow-slate-900/5 ring-slate-500/10">
              <image_1.default className="w-full" src={feature.image} alt="" sizes="52.75rem"/>
            </div>
          </div>
        </div>))}
    </div>);
...
function FeaturesDesktop() {
    return (<react_2.TabGroup className="hidden lg:mt-20 lg:block">
      {({ selectedIndex }) => (<>
          <react_2.TabList className="grid grid-cols-3 gap-x-8">
            {features.map((feature, featureIndex) => (<Feature key={feature.summary} feature={{
                    ...feature,
                    name: (<react_2.Tab className="data-selected:not-data-focus:outline-hidden">
                      <span className="absolute inset-0"/>
                      {feature.name}
                    </react_2.Tab>),
                }} isActive={featureIndex === selectedIndex} className="relative"/>))}
          </react_2.TabList>
          <react_2.TabPanels className="relative mt-20 overflow-hidden rounded-4xl bg-slate-200 px-14 py-16 xl:px-16">
            <div className="-mx-5 flex">
              {features.map((feature, featureIndex) => (<react_2.TabPanel static key={feature.summary} className={(0, clsx_1.default)('px-5 transition duration-500 ease-in-out data-selected:not-data-focus:outline-hidden', featureIndex !== selectedIndex && 'opacity-60')} style={{ transform: `translateX(-${selectedIndex * 100}%)` }} aria-hidden={featureIndex !== selectedIndex}>
...
function SecondaryFeatures() {
    return (<section id="secondary-features" aria-label="Features for simplifying everyday business tasks" className="pt-20 pb-14 sm:pt-32 sm:pb-20 lg:pb-32">
...
exports.SecondaryFeatures = SecondaryFeatures;
