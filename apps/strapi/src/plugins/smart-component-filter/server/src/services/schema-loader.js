...
exports.default = ({ strapi }) => ({
     ...
     */
    async loadSchemaFromListingType(listingTypeId) {
        try {
     ...
     */
    extractComponentNames(fieldGroup) {
        if (!fieldGroup)
            return [];
        ...
        if (typeof fieldGroup === 'object') {
            Object.values(fieldGroup).forEach((value) => {
     ...
     */
    async loadComponentSchemas(componentNames) {
        const schemas = {};
     ...
     */
    async validateFieldData(data, schema) {
        const errors = [];
        if (!data || !schema) {
        ...
        try {
            ...
            for (const [componentName, componentData] of Object.entries(data)) {
                ...
                const attributeErrors = this.validateComponentAttributes(componentData, componentSchema.attributes);
     ...
     * Validate component attributes
     */
    validateComponentAttributes(data, attributes) {
        const errors = [];
     ...
     */
    async getAvailableComponents(categoryId) {
        try {
