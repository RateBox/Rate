...
const ComponentFilterNew: React.FC = () => {
  ...
  const componentPickerObserver = useRef<MutationObserver | null>(null);
  ...
  const applyComponentFilter = (allowedComponents: string[]) => {
    ...
    allCategoryContainers.forEach((category) => {
      const categoryContainer = (category as HTMLElement).closest('div[class*="accordion"]') || (category as HTMLElement).parentElement;
    ...
    allowedComponents.forEach(componentPath => {
      ...
      const categoryHeading = Array.from(allCategoryContainers).find((el) => 
        el.textContent?.toLowerCase().includes(category.toLowerCase())
      );
      
      if (categoryHeading) {
        const categoryContainer = (categoryHeading as HTMLElement).closest('div[class*="accordion"]') || (categoryHeading as HTMLElement).parentElement;
        if (categoryContainer) {
          (categoryContainer as HTMLElement).style.display = 'block';
          visibleCount++;
  ...
  const setupComponentPickerObserver = () => {
    if (componentPickerObserver.current) return;
        ...
        mutation.addedNodes.forEach((node: any) => {
          if (node.nodeType === 1 && node.getAttribute?.('role') === 'dialog') {
            ...
            if (selectedListingType) {
                 ...
                 const allowedComponents = mapping[selectedListingType.Name as keyof typeof mapping] || [];
  ...
  const fetchListingTypeData = async (listingTypeId: string) => {
    try {
      ...
      if (listingTypeContainer) {
        ...
        if (selectedChip) {
          ...
          if (listingTypeName && (!selectedListingType || selectedListingType.Name !== listingTypeName)) {
            ...
            const listingTypeId = (hiddenInput as HTMLInputElement)?.value;
  ...
  return (
    <div style={{ padding: '16px', backgroundColor: '#f8f9fa', borderRadius: '8px', marginTop: '16px' }}>
      <h3 style={{ margin: '0 0 12px 0', color: '#333', fontSize: '14px', fontWeight: 'bold' }}>
        🧠 SMART COMPONENT FILTER (v2.0)
      </h3>
      
      {selectedListingType ? (
        <div style={{ marginBottom: '12px', padding: '8px', backgroundColor: '#e8f5e8', borderRadius: '4px' }}>
      ...
      <div style={{ fontSize: '11px', color: '#555', maxHeight: '120px', overflowY: 'auto' }}>
        {debugInfo.map((info, index) => (
          <div key={index} style={{ marginBottom: '2px', fontFamily: 'monospace' }}>
            {info}
          </div>
        ))}
      </div>
...
export default ComponentFilterNew; 