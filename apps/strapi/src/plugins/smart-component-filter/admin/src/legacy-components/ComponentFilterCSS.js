...
const ComponentFilterCSS = () => {
    ...
    const applyDOMFilter = (listingType) => {
        ...
        const isInComponentModal = (element) => {
            ...
            groupsToHide.forEach(groupName => {
                ...
                headings.forEach(heading => {
                    if (heading.textContent?.toLowerCase().includes(groupName) && isInComponentModal(heading)) {
                        ...
                        let groupContainer = heading.closest('div[role="region"], div:has(h3), section, article');
                        if (!groupContainer) {
...
exports.default = ComponentFilterCSS;
