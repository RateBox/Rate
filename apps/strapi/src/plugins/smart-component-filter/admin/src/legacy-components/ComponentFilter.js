...
const ComponentFilter = () => {
    ...
    const waitForComponentPicker = (callback, maxAttempts = 50) => {
        let attempts = 0;
    ...
    const fetchListingTypeById = async (listingTypeId) => {
        ...
        try {
            ...
            itemGroup.forEach((componentUID) => {
            ...
            reviewGroup.forEach((componentUID) => {
                const [category] = componentUID.split('.');
                if (!componentMap.has(category)) {
                    componentMap.set(category, []);
                }
    ...
    const applyComponentFiltering = (componentMap) => {
        ...
        componentGroups.forEach((heading) => {
            ...
            const shouldShow = allowedGroups.some(group => textContent.includes(group.toLowerCase()) ||
                group.toLowerCase().includes(textContent));
            // Find the container to hide/show
            let container = heading.parentElement;
            // Try different container strategies
            if (!container)
                container = heading.closest('div, section, article');
            if (container) {
                if (shouldShow) {
            ...
            if (newGroups.length > 0) {
                ...
                newGroups.forEach(group => {
                    ...
                    if (knownGroups.includes(textContent)) {
                        ...
                        const shouldShow = allowedGroups.some(allowed => textContent.includes(allowed.toLowerCase()) ||
                            allowed.toLowerCase().includes(textContent));
                        const container = group.parentElement || group.closest('div');
    ...
    if (loading) {
        return (<div style={{ padding: '16px', backgroundColor: '#f6f6f9', borderRadius: '4px', margin: '16px 0' }}>
    ...
    if (!filterData) {
        return (<div style={{ padding: '16px', backgroundColor: '#fdf4f4', borderRadius: '4px', margin: '16px 0' }}>
