...
const HomePage = () => {
  ...
  const [reviewComponents, setReviewComponents] = useState<string[]>([]);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (contentTypesRes.data) {
          ...
          console.log('📊 All UIDs:', allContentTypes.map((type: any) => type.uid));
          
          const componentTypes = allContentTypes.filter((type: any) => 
            type.uid && type.uid.startsWith('components.')
          );
          
          console.log('🧩 Found components:', componentTypes.length);
          console.log('🧩 Component UIDs:', componentTypes.map((c: any) => c.uid));
  ...
  const handleListingTypeChange = async (listingTypeId: string) => {
    ...
    try {
      const response = await getFetchClient().get(`/admin/plugins/_smart-component-filter/listing-type/${listingTypeId}/components`);
  ...
  const handleSaveConfiguration = async () => {
    ...
    setSaving(true);
    try {
      const response = await getFetchClient().post(`/admin/plugins/_smart-component-filter/listing-type/${selectedListingType}/components`, {
  ...
  const handleComponentToggle = (componentUid: string, type: 'item' | 'review') => {
    if (type === 'item') {
  ...
  return (
    <Page.Main>
      <Page.Title>Smart Component Filter Configuration</Page.Title>
        ...
        <div style={{ padding: '24px' }}>
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <select 
              ...
              title="Select a listing type to configure component filtering"
              style={{ 
                padding: '12px', 
                borderRadius: '6px', 
          ...
          {selectedListingType && (
            <>
              <div style={{ 
                ...
                <h3>📝 Configuration for "{listingTypes.find((lt: any) => lt.id.toString() === selectedListingType)?.name}"</h3>
