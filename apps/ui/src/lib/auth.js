    ...
    providers: [
        (0, credentials_1.default)({
            ...
            async authorize(credentials) {
                ...
                return strapi_api_1.PrivateStrapiClient.fetchAPI(`/auth/local`, undefined, {
                    body: JSON.stringify({
    ...
    callbacks: {
        jwt: async ({ token, user, trigger, account, session }) => {
            ...
            if (account) {
                ...
                if (account.access_token != null) {
                    ...
                    try {
                        const data = await strapi_api_1.PrivateStrapiClient.fetchAPI(`/auth/${account.provider}/callback?access_token=${account.access_token}`, undefined, { next: { revalidate: 0 } });
                        const { jwt, user } = data;
                        if (jwt == null) {
        ...
        session: async ({ token, session }) => {
            if (token?.strapiJWT != null && token?.error == null) {
                ...
                try {
                    const fetchedUser = await strapi_api_1.PrivateStrapiClient.fetchAPI("/users/me", undefined, { next: { revalidate: 0 } }, { userJWT: token.strapiJWT });
...
function getAuth(...args) {
    return (0, next_auth_1.getServerSession)(...args, exports.authOptions);
}
