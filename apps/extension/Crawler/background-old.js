...
async function submitShopeeReviewsToStrapi(reviews) {
  ...
  try {
    ...
    // Check for successful submission - new format
    if (result.success === true) {
      console.log('[Background] Successfully submitted to Strapi:', result);
      
      // Save submission info
      await chrome.storage.local.set({
        lastValidationRequestId: result.requestId || result.listingId || Date.now().toString(),
        lastValidationTimestamp: new Date().toISOString(),
        lastValidationQueued: result.queued || false
      });
      
      // IMPORTANT: Clear submitted reviews to prevent duplicate submissions
      console.log('[Background] Clearing submitted reviews from database...');
      ...
      const errorMsg = result.error || (result.errors && result.errors.join(', ')) || 'Unknown error';
      throw new Error(`Strapi error: ${errorMsg}`);
    } else if (result.requestId) {
      // Legacy format - for backward compatibility
      console.log('[Background] Successfully submitted to Strapi Redis Stream (legacy):', result);
...
async function handleShopeeReviewsFound(message, sender) {
  try {
    ...
    const newUrl = data[0]?.product?.productUrl;
    
    // Clear old data if it's a new product (different URL)
    if (shopeeReviewsDatabase.length > 0) {
      const existingUrl = shopeeReviewsDatabase[0]?.product?.productUrl;
      if (existingUrl && newUrl && existingUrl !== newUrl) {
        console.log('[Background] New product detected, clearing old reviews');
        shopeeReviewsDatabase = [];
      }
    }
    
    // IMPORTANT: Don't add duplicate reviews from same page reload
    // Check if we already have reviews with same content to prevent duplicates from page reloads
    const existingReviewIds = new Set(shopeeReviewsDatabase.map(r => r.id || r.userId || ''));
    const existingReviewContents = new Set(shopeeReviewsDatabase.map(r => 
    ...
    const newReviews = data.filter(review => {
      ...
      if (existingReviewContents.has(reviewKey)) {
        return false;
      }
      return true;
    });
    ...
    if (newReviews.length === 0) {
      console.log('[Background] No new reviews to add (all duplicates from reload)');
      return;
    }
    
    // Add only new reviews
    shopeeReviewsDatabase = [...shopeeReviewsDatabase, ...newReviews];
    totalShopeeReviews = shopeeReviewsDatabase.length;
    console.log(`[Background] Added ${newReviews.length} new reviews, total: ${shopeeReviewsDatabase.length}`);
    updateBadge();
    await saveAccumulatedData();
